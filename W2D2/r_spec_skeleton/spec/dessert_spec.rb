...
describe Dessert do
  let(:chef) { double("chef", name: "Rick") }
  ...
  describe "#initialize" do
    ...
    it "raises an argument error when given a non-integer quantity" do
      expect { Dessert.new("brownie", "hella", "Rick") }.to raise_error(ArgumentError)
    end
  end
  ...
  describe "#mix!" do
    it "shuffles the ingredient array" do
      ingredients = ["eggs", "flour", "butter", "milk", "chocolate"]
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      allow(chef).to receive(:titleize).and_return("Chef Rick the Great Baker")
      expect(brownie.serve).to eq("Chef Rick the Great Baker has made 100 brownies!")
    end
  end
